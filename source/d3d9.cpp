...
/* D3D draw stuffs */
D3DRECT bg30Rect, bg60Rect, bg300Rect, borderRect;
...
bool WINAPI DllMain(HMODULE hModule, DWORD fdwReason, LPVOID lpReserved) {
	switch (fdwReason) {
		case DLL_PROCESS_ATTACH: {
			...
			blurAPI->unload(); //dont leak mem today
			FreeLibrary(hModule);
			break;
		}
	}
...
HRESULT f_IDirect3DDevice9::Present(CONST RECT *pSourceRect, CONST RECT *pDestRect, HWND hDestWindowOverride, CONST RGNDATA *pDirtyRegion) {
	...
	if (bFPSLimit) {
		...
		double targetFrameTime = 1000.0 / fFPSLimit; //set to non-static so we can modify the target during gameplay
		static double t = 0.0;
		static DWORD i = 0;
		...
		while (true) {
			...
			if (t == 0.0) {
				...
				if (!bOnce2) {
					...
					//biggest positive number that will fit in 32 bits using Ca2
...
HRESULT f_iD3D9::CreateDevice(UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS *pPresentationParameters, IDirect3DDevice9 **ppReturnedDeviceInterface) {
	...
	*ppReturnedDeviceInterface = *temp;
	...
	borderRect = {bg30Rect.x1-1,bg30Rect.y1-1, bg300Rect.x2+1, bg300Rect.y2+1};
...
HRESULT f_IDirect3DDevice9::EndScene() {
	...
	static int iFpsTarget = 1;
	static float fFpsTargetValues[] = {5.0, 30.0, 60.0, 100.0, 300.0};
	static const int countTargets = sizeof(fFpsTargetValues) / sizeof(fFpsTargetValues[0]);

	// NOTE: draw your custom D3D components here.
	bool bFPSLimit = blurAPI->config.bFPSLimit;
	float fFPSLimit = blurAPI->config.fps;
	bDrawBox = GetKeyState(VK_END);
	if (bDrawBox) {
		bFPSLimit = true;
		borderRect.x2 = bg30Rect.x1 + ((int)fFPSLimit);
		f_pD3DDevice->Clear(1, &borderRect, D3DCLEAR_TARGET, D3DCOLOR_XRGB(255, 0, 0), 0, 0);
		f_pD3DDevice->Clear(1, &bg60Rect, D3DCLEAR_TARGET, D3DCOLOR_XRGB(0, 150, 255), 0, 0);
		f_pD3DDevice->Clear(1, &bg30Rect, D3DCLEAR_TARGET, D3DCOLOR_XRGB(0, 0, 255), 0, 0);
		bDrawBox = false;
	}

	if (GetKeyState(VK_ADD) < 0) {
		fFPSLimit = min(fFPSLimit + 1, fFpsTargetValues[countTargets-1]);
		bPressed = true;
	} else if (GetKeyState(VK_SUBTRACT) < 0) {
		fFPSLimit = max(fFPSLimit - 1, fFpsTargetValues[0]);
		bPressed = true;
	} else if ((GetKeyState(VK_MULTIPLY) < 0)) {
		if (!bPressed) {
			/* find prev target as if it was moved, just in case */
			for (int i = 0; i < countTargets; i++) {
				if (fFPSLimit < fFpsTargetValues[i]) {
					iFpsTarget = i-1;
					break;
				}
			}
			iFpsTarget = (iFpsTarget + 1) % countTargets;
			fFPSLimit = fFpsTargetValues[iFpsTarget];
		}
		bPressed = true;
